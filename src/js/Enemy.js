export default class Enemy {
    ...
    for (let i = 0; i < this.enemies.length; i += 1) {
      attackIndexes
        .push(...this.enemies[i].character
          .attackRangeIndex(this.enemies[i].position, this.boardSize));
    }
  ...
  static attackIndex(enemy, indexes) {
    for (let i = 0; i < indexes.length; i += 1) {
      const box = enemy.attackRangeIndex(indexes[i], this.boardSize)
  ...
  static move() {
    ...
    for (let i = 0; i < this.enemies.length; i += 1) {
      const indexesMovement = this.enemies[i].character
        .movementRangeIndex(this.enemies[i].position, this.boardSize)
        .filter((el) => !posBusy.includes(el));
      const attackIndex = this.attackIndex(this.enemies[i].character, indexesMovement);
    ...
    const box = maxAttacker.character
      .movementRangeIndex(maxAttacker.position, this.boardSize)
      .filter((el) => !posBusy.includes(el));
